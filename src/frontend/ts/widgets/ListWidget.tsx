...
import {AddResponseMessage} from "../../../shared/messages/AddResponseMessage";
		...
		return <form onsubmit={ async () => {
		...
		}} class="vertical">
			{
				vNode.attrs.columns.map((column) => <label>
					...
					{ this.getTypedInputView(this.data, obj, column) }
				</label>)
			}
			{ LoadingSpinner(this.isLoading) }
			<input disabled={this.isLoading} type="submit" value={Lang.get("add")}/>
		</form>;
	}
